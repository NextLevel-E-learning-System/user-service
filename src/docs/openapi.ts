...
export const openapiSpec = {
  ...
  paths: {
    ...
    '/users/v1/departamentos/{codigo}': { get: { summary: 'Obter departamento (ADMIN)', tags: ['User - Departamentos'], security: [{ bearerAuth: [] }], parameters: [{ name: 'codigo', in: 'path', required: true, schema: { type: 'string' } }], responses: { '200': { description: 'Departamento', content: { 'application/json': { schema: { type: 'object', properties: { departamento: { $ref: '#/components/schemas/DepartamentoCompleto' }, mensagem: { type: 'string' } } } } } }, '404': { description: 'Não encontrado', content: { 'application/json': { schema: { $ref: '#/components/schemas/ErrorResponse' } } } } } } },
    '/users/v1/cargos': { get: { summary: 'Listar cargos (público)', tags: ['User - Cargos'], responses: { '200': { description: 'Lista', content: { 'application/json': { schema: { type: 'object', properties: { items: { type: 'array', items: { $ref: '#/components/schemas/Cargo' } }, mensagem: { type: 'string' } } } } } } } }, post: { summary: 'Criar cargo (ADMIN)', tags: ['User - Cargos'], security: [{ bearerAuth: [] }], requestBody: { required: true, content: { 'application/json': { schema: { $ref: '#/components/schemas/CargoCreate' } } } }, responses: { '201': { description: 'Criado', content: { 'application/json': { schema: { type: 'object', properties: { cargo: { $ref: '#/components/schemas/Cargo' }, mensagem: { type: 'string' } } } } } }, '409': { description: 'Conflito', content: { 'application/json': { schema: { $ref: '#/components/schemas/ErrorResponse' } } } } } } },
    ...
    '/users/v1/funcionarios/{id}/role': { put: { summary: 'Atualizar role (ADMIN)', tags: ['User - Funcion�rios'], security: [{ bearerAuth: [] }], parameters: [{ name: 'id', in: 'path', required: true, schema: { type: 'string', format: 'uuid' } }], requestBody: { required: true, content: { 'application/json': { schema: { type: 'object', required: ['role'], properties: { role: { type: 'string', enum: ['ADMIN', 'INSTRUTOR', 'GERENTE', 'ALUNO'] } } } } } }, responses: { '200': { description: 'Atualizada', content: { 'application/json': { schema: { type: 'object', properties: { funcionario: { $ref: '#/components/schemas/Funcionario' }, mensagem: { type: 'string' } } } } } }, '404': { description: 'Não encontrado', content: { 'application/json': { schema: { $ref: '#/components/schemas/ErrorResponse' } } } } } } },
  ...
  components: {
    ...
    schemas: {
      ...
      CargoCreate: { type: 'object', required: ['codigo', 'nome'], properties: { codigo: { type: 'string' }, nome: { type: 'string' } } },
      CargoUpdate: { type: 'object', properties: { nome: { type: 'string' } } },
      Funcionario: { type: 'object', properties: { id: { type: 'string', format: 'uuid' }, nome: { type: 'string' }, email: { type: 'string', format: 'email' }, role: { type: 'string', enum: ['ADMIN', 'INSTRUTOR', 'GERENTE', 'ALUNO'] } }, required: ['id', 'nome', 'email', 'role'] },
      FuncionarioRegister: { type: 'object', required: ['nome', 'email'], properties: { nome: { type: 'string' }, email: { type: 'string', format: 'email' }, role: { type: 'string', enum: ['ADMIN', 'INSTRUTOR', 'GERENTE', 'ALUNO'], default: 'ALUNO' } } },
