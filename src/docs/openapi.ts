...
export const openapiSpec = {
  ...
  paths: {
    ...
    '/users/v1/funcionarios/{id}/role': { put: { summary: 'Atualizar role (ADMIN)', tags: ['User - Funcion�rios'], security: [{ bearerAuth: [] }], parameters: [{ name: 'id', in: 'path', required: true, schema: { type: 'string', format: 'uuid' } }], requestBody: { required: true, content: { 'application/json': { schema: { type: 'object', required: ['role'], properties: { role: { type: 'string', enum: ['ADMIN', 'INSTRUTOR', 'GERENTE', 'FUNCIONARIO'] } } } } } }, responses: { '200': { description: 'Atualizada', content: { 'application/json': { schema: { type: 'object', properties: { funcionario: { $ref: '#/components/schemas/Funcionario' }, mensagem: { type: 'string' } } } } } }, '404': { description: 'Não encontrado', content: { 'application/json': { schema: { $ref: '#/components/schemas/ErrorResponse' } } } } } } },
